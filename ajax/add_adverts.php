...
if($get_header==""){
    ...
    echo " <b><i class='fa fa-exclamation-circle' aria-hidden='true'></i> &nbsp;oops! please be sure that this is an jpg,jpeg or png file</b>";
}else {
    $temp = explode(".", $file_name);
        $newfilename = round(microtime(true)) . '.' . end($temp);
 $sql_insert = "INSERT INTO advert(header,recent_time,file_name,archive_format,url_entered) VALUES('$get_header','$time','$newfilename',
 '$combine','$url')";
 $run_query = $conn->query($sql_insert);
 if($run_query){
     move_uploaded_file($tmp_name, "../assets/adverts/$newfilename");
     echo "<b><i class='fa fa-check-user-o'></i> this advert has been successfully uploaded</b>";
 }
