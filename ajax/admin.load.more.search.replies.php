...
if($get_var==""){
 ...
 if($space_checker==0){
    ...
    $sql = "SELECT * FROM replies_table where id > '$id' AND post_content LIKE '%$get_var%' LIMIT 3";
...
if($rowcount == 0){
 ...
 foreach ($value_explode as $key => $value) {
     $sql = "SELECT * FROM replies_table where id > '$id' AND post_content LIKE '%$get_var%' OR '%$value%' ";
 }
     $sql.="LIMIT 3";
