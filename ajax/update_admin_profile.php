...
@$cookie_value =  $_COOKIE[$conn->cookie_name];
if($username==""){
    ...
    $check_email = "SELECT * FROM admin where email='$email' AND email!='$hidden_value'";
      ...
      if($return_total > 0){
        ...
        $newfilename = round(microtime(true)) . '.' . end($temp);
            $sql_insert = "
            UPDATE admin SET username='$username', password='$password_hash',confirm='$confirm_hash',image='$newfilename',
            description = '$message',tmp_password='$tmp_password' where email='$cookie_value'";
...
echo"<b><i class='fa fa-check-circle-o' aria-hidden='true'></i>&nbsp;new user registered successfully</b>";
            $conn->destroycookie();
            setcookie($conn->cookie_name, $email, time() + (86400 * 30), "/");
        }
